...
function drawParticles() {
    particles.forEach(p => {
        ...
        if (p.type === 'Z') {
            ...
            // Draw simple pixel musical note
...
function drawBackground() {
    ...
    // Floor line
    ctx.strokeStyle = themeColors.medium;
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(0, 130);
    ...
    // Bed (Left side)
    // Pillow
    ctx.fillStyle = themeColors.medium;
    ctx.fillRect(15, 110, 12, 8);
    ctx.fillStyle = themeColors.light;
    ctx.fillRect(17, 111, 8, 6);
    // Bed frame/sheet
    ctx.fillStyle = themeColors.dark;
    ...
    // Desk
    ctx.fillStyle = themeColors.dark;
    ctx.fillRect(250, 115, 55, 15);
    ctx.fillStyle = themeColors.medium;
    ctx.fillRect(252, 116, 51, 13);
    // Desk legs
    ctx.fillStyle = themeColors.dark;
    ...
    // Computer Monitor
    ctx.fillStyle = themeColors.dark;
    ctx.fillRect(265, 94, 26, 20);
    ctx.fillStyle = themeColors.light;
    ctx.fillRect(267, 96, 22, 15);
    // Stand
    ctx.fillStyle = themeColors.dark;
    ctx.fillRect(275, 114, 6, 2);
    // Computer screen display glow (glowing lines if working)
    if (state.agent.state === 'WORKING') {
        ctx.fillStyle = themeColors.dark;
        ctx.fillRect(269, 98, 18, 2);
        ctx.fillRect(269, 102, 14, 2);
        ctx.fillRect(269, 106, 8, 2);
    }

    // Chair
    ctx.fillStyle = themeColors.dark;
    ctx.fillRect(234, 120, 12, 18); // Seat and leg
    ctx.fillRect(230, 118, 12, 2); // Cushion
    ctx.fillRect(230, 105, 2, 14); // Backrest
}
...
function drawRobotSprite(x, y, flip, workPose, tick, walkAlt = false) {
    ...
    // 1. Antenna
    ctx.fillRect(-1, -22, 2, 4); // Pole
    ctx.fillRect(-2, -24, 4, 2); // Tip
    ...
    // Face/Eyes
    ctx.fillStyle = themeColors.dark;
    const isBlink = Math.floor(tick / 120) % 20 === 0; // Blink every few seconds
    if (isBlink && !workPose) {
        // Closed eyes (lines)
    ...
    if (workPose) {
        ...
        // Joy arms up
        ctx.fillRect(-10, -5, 3, 6); // Left arm up
        ctx.fillRect(7, -5, 3, 6);  // Right arm up
    } else {
        // Idle dangling arms
        ctx.fillRect(-10, -2, 3, 6); // Left
        ctx.fillRect(7, -2, 3, 6);  // Right
    }

    // 6. Legs & Feet
    ctx.fillStyle = themeColors.dark;
    if (walkAlt) {
        // Walk pose left leg up, right leg down
        ctx.fillRect(-5, 6, 3, 5); // L
        ctx.fillRect(2, 6, 3, 3);  // R
    } else if (state.agent.state === 'WALKING') {
        // Walk pose alt
        ctx.fillRect(-5, 6, 3, 3); // L
        ctx.fillRect(2, 6, 3, 5);  // R
    } else {
        // Standing legs
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    chatForm.addEventListener('submit', (e) => {
        ...
        // Bot response typing effect delay
        setTimeout(() => {
